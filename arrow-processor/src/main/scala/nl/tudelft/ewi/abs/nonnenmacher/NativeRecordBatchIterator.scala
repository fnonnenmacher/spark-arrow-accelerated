...
object NativeRecordBatchIterator {
  ...
  def apply(fileName: String, fieldNames: Array[Int]): Iterator[Array[Byte]] = {
    new NativeRecordBatchIterator(initializer.init(fileName, fieldNames)).asScala
  };

  private class Initializer {

    @native def init(fileName: String, fieldNames: Array[Int]): Long;
  }
