...
class ParqueteToPlasmaReader : Iterator<plasma::ObjectID>{
private:
    ...
    unique_ptr<BatchProcessor<std::shared_ptr<arrow::Buffer>, plasma::ObjectID>> buffer_to_plasma_processor;

public:
    explicit ParqueteToPlasmaReader(const char* file_name);
    bool hasNext() override;
    plasma::ObjectID next() override;
