...
JNIEXPORT void JNICALL Java_nl_tudelft_ewi_abs_nonnenmacher_ArrowProcessorJni_readParquete(JNIEnv * env,
        jobject, jstring java_file_name, jbyteArray object_id_java_array) {

    ObjectID object_id_out = objectIdFromJavaArray(env, object_id_java_array);
    std::cout << "object_id_out is " << object_id_out.hex() << std::endl;

    const char *file_path = env->GetStringUTFChars(java_file_name, 0);

    std::unique_ptr<parquet::arrow::FileReader> reader;
    PARQUET_THROW_NOT_OK(parquet::arrow::FileReader::Make(arrow::default_memory_pool(),
                                                            parquet::ParquetFileReader::OpenFile(file_path),
                                                            &reader));

      env->ReleaseStringUTFChars(java_file_name, file_path);

      std::shared_ptr<::arrow::Table> table;
      PARQUET_THROW_NOT_OK(reader->ReadTable(&table));

    auto sink = arrow::io::BufferOutputStream::Create().ValueOrDie();
    std::shared_ptr<arrow::ipc::RecordBatchWriter> rb_writer = arrow::ipc::NewStreamWriter(sink.get(),
                                                                                         table->schema()).ValueOrDie();

    PARQUET_THROW_NOT_OK(rb_writer->WriteTable(*table));
    PARQUET_THROW_NOT_OK(rb_writer->Close());
    ...
    std::cout << "Plasma client created result object: " << s1.ok() << std::endl;
    arrow::Status s3 = client.Seal(object_id_out);
    std::cout << "Plasma client wrote result (" << data_buffer->size() << " bytes) to the plasma store" << std::endl;
}